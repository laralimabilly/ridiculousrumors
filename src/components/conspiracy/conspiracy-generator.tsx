// components/conspiracy/conspiracy-generator.tsx - Mobile Responsive Version
'use client';

import React, { useState, useEffect } from 'react';
import { RefreshCw, FileText, Zap, Clock, User, Eye, Globe, MessageCircle, Share2, Copy, Check, Download, EyeOff, AlertTriangle, ExternalLink, Menu, X } from 'lucide-react';
...
// Inline Mobile-Responsive CategoryCard component
...
const CategoryCard: React.FC<CategoryCardProps> = ({ 
  ...
  <button
    ...
    disabled={disabled}
    className={`
      relative group border-2 bg-gray-900/80 backdrop-blur-sm rounded-none 
      ${isMobile ? 'p-3' : 'p-6'}
      transition-all duration-300 font-mono
      disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100
      ${isMobile ? 'min-h-[80px]' : 'hover:scale-105'}
      ${isSelected 
  ...
  >
    ...
    <div className="flex flex-col items-center text-center relative z-10">
      <div className={`
        ${isMobile ? 'mb-2 p-2' : 'mb-4 p-3'} 
        border border-current rounded bg-gradient-to-r text-black font-bold 
        transition-transform duration-300 group-hover:scale-110 ${category.color}
      `}>
        {category.icon}
      </div>

      <div className={`
        text-green-300 font-bold ${isMobile ? 'text-xs' : 'text-sm'} 
        mb-1 md:mb-2 tracking-wider
      `}>
        [ {category.label.replace(/\s/g, '_').toUpperCase()} ]
      </div>

      <div className="text-green-400/70 text-xs">
        {isSelected ? '> ACTIVATED' : '> SELECT'}
      </div>
    ...
    {isSelected && (
      <>
        ...
        <div className="absolute top-1 right-1 text-green-400 text-xs font-bold animate-blink">
          ACTIVE
        </div>
...
// Mobile-Responsive TheoryDisplay component
...
const TheoryDisplay: React.FC<TheoryDisplayProps> = ({ theory, onShare, onSave, isMobile = false }) => {
  ...
  const shareButtons = [
    { id: 'facebook' as SharePlatform, label: isMobile ? 'FB' : 'FACEBOOK_LEAK', color: 'blue', icon: '📘' },
    { id: 'twitter' as SharePlatform, label: isMobile ? 'X' : 'TWITTER_DUMP', color: 'sky', icon: '🐦' },
    { id: 'reddit' as SharePlatform, label: isMobile ? 'RED' : 'REDDIT_POST', color: 'orange', icon: '🤖' }
  ];

  return (
    <div className={`
      bg-black border-2 rounded-none 
      ${isMobile ? 'p-3' : 'p-6'} 
      relative font-mono animate-fadeIn ${getClassificationColor(theory.classification)}
    `}>
      <div className={`
        absolute top-0 left-0 right-0 text-center py-1 border-b 
        ${isMobile ? 'text-xs' : 'text-xs'} 
        font-bold tracking-widest 
        ${theory.classification === 'TOP SECRET' ? 'bg-red-900/50 border-red-400' : 'bg-orange-900/50 border-orange-400'}
      `}>
        {isMobile 
          ? `${theory.classification} - FICTIONAL` 
          : `${theory.classification} - UNAUTHORIZED DISCLOSURE PROHIBITED - RIDICULOUS RUMORS COMEDY DIVISION`
        }
      </div>
      
      <div className={`${isMobile ? 'mt-6 mb-3' : 'mt-8 mb-6'}`}>
        <div className="flex justify-between items-start mb-2 md:mb-4">
          <div className="flex-1">
            <div className={`
              ${isMobile ? 'text-sm' : 'text-lg'} 
              font-bold tracking-wider mb-1 md:mb-2
            `}>
              &gt;&gt; INTEL REPORT {isMobile ? '' : 'DECLASSIFIED'} &lt;&lt;
            </div>
            <div className="text-xs opacity-70 space-y-1">
              <div>ORIGIN: DEEP_THROAT_7 | CLEARANCE: COMEDY</div>
              <div>DATE: {formatDate(theory.createdAt)} | CAT: {theory.category.toUpperCase()}</div>
              {!isMobile && <div>DOCUMENT_ID: {theory.id || 'UNASSIGNED'}</div>}
            </div>
          </div>
          ...
          <button
            ...
            className={`
              ...
              border border-current rounded-none hover:bg-current hover:text-black transition-colors
            `}
          >
            {isRedacted ? <Eye className="w-3 h-3" /> : <EyeOff className="w-3 h-3" />}
            <span className="hidden md:inline">{isRedacted ? 'UNREDACT' : 'REDACT'}</span>
          </button>
        </div>
        
        <div className="border-t border-current/30 pt-2 md:pt-4">
          <div className={`
            text-green-300 leading-relaxed ${isMobile ? 'text-sm' : 'text-lg'} 
            whitespace-pre-wrap text-center font-bold
          `}>
            {isRedacted ? (
              <div className="flex items-center justify-center gap-2">
                <div className="bg-black text-black select-none">
                  {'█'.repeat(Math.min(theory.content.length, isMobile ? 20 : 30))}
                </div>
      ...
      {/* Disclaimer */}
      <div className={`
        bg-yellow-900/20 border border-yellow-400/50 rounded-none 
        ${isMobile ? 'p-2 mb-3' : 'p-3 mb-4'}
      `}>
        <div className="flex items-center gap-2 text-yellow-400 text-xs font-bold mb-1">
          <AlertTriangle className="w-3 h-3" />
          {isMobile ? 'SATIRICAL WARNING' : 'SATIRICAL CONTENT WARNING'}
        </div>
        <div className="text-yellow-300 text-xs">
          This theory is completely fictional and generated for entertainment purposes only.
          {!isMobile && ' It does not represent real beliefs or facts. Ridiculous Rumors creates comedy content only.'}
        </div>
      </div>
      ...
      {isMobile && (
        <button
          ...
          className="w-full flex items-center justify-center gap-2 mb-3 px-4 py-2 border border-current rounded-none text-xs hover:bg-current hover:text-black transition-colors"
        >
          {showActions ? <X className="w-3 h-3" /> : <Menu className="w-3 h-3" />}
      ...
      {/* Actions Section */}
      {showActions && (
        <div className="border-t border-current/30 pt-2 md:pt-4 space-y-3 md:space-y-4">
          <div className={`
            flex ${isMobile ? 'flex-col space-y-2' : 'justify-between items-center'}
          `}>
            <button
              onClick={handleCopyToClipboard}
              className={`
                flex items-center gap-2 ${isMobile ? 'justify-center w-full' : ''} 
                px-4 py-2 border border-current rounded-none text-xs 
            ...
            {theory.id && (
              <button
                ...
                className={`
                  flex items-center gap-2 ${isMobile ? 'justify-center w-full' : ''} 
                  px-4 py-2 border-2 border-green-400 bg-green-900/20 text-green-400 
                  hover:bg-green-900/40 transition-colors rounded-none text-xs font-bold
                `}
              >
                <ExternalLink className="w-3 h-3" />
                {isMobile ? 'VIEW_FULL' : 'VIEW_FULL_REPORT'}
              </button>
            )}
          </div>

          <div>
            <div className="font-bold mb-2 md:mb-4 flex items-center gap-2 text-xs md:text-sm">
              <Share2 className="w-4 h-4" />
              {isMobile ? 'SHARE:' : 'INITIATE_PUBLIC_LEAK_PROTOCOL'}
            </div>
            <div className={`
              grid ${isMobile ? 'grid-cols-3' : 'grid-cols-3'} gap-2 md:gap-3
            `}>
              {shareButtons.map((platform) => (
                <button
                  ...
                  className={`
                    ...
                    border-2 rounded-none font-mono text-xs 
                    hover:scale-105 transition-all duration-300 
                    border-${platform.color}-400 text-${platform.color}-400 
                    bg-${platform.color}-900/20 hover:bg-${platform.color}-900/40
                  `}
                >
                  {isMobile ? (
                    <>
...
const ConspiracyGenerator: React.FC<ConspiracyGeneratorProps> = ({
  ...
  return (
    <div className="space-y-4 md:space-y-8">
      ...
      <div className="border border-green-400/30 rounded-none p-2 md:p-4 bg-gray-900/50">
        ...
        <div className={`
          mt-2 grid ${isMobile ? 'grid-cols-1' : 'grid-cols-2'} gap-2 md:gap-4 text-xs font-mono
        `}>
      ...
      <div className="text-center mb-4 md:mb-8">
        <div className={`
          text-green-400 font-bold ${isMobile ? 'text-lg' : 'text-xl'} 
          mb-2 tracking-wider font-mono
        `}>
      ...
      {selectedCategory && !isGenerating && (
        <div className="text-center mb-4 md:mb-6">
          <div className={`
            ...
            border border-green-400/50 bg-green-900/20 rounded-none font-mono
          `}>
      ...
      {error && (
        <div className="border border-red-400 bg-red-900/20 rounded-none p-2 md:p-4 mb-4 md:mb-6">
      ...
      <div className="text-center mb-4 md:mb-8">
        <button
          ...
          className={`
            relative ${isMobile ? 'w-full px-6 py-3' : 'px-8 py-4'} 
            font-bold border-2 rounded-none font-mono text-sm tracking-wider
          ...
          <div className="flex items-center justify-center gap-2 md:gap-3">
            {isGenerating ? (
              <>
                ...
                {isMobile ? 'ACCESSING...' : 'ACCESSING_CLASSIFIED_FILES...'}
              </>
            ) : (
              <>
                <FileText className="w-4 md:w-5 h-4 md:h-5" />
                {isMobile ? 'EXECUTE_PROTOCOL' : 'EXECUTE_DECLASSIFICATION_PROTOCOL'}
              </>
            )}
          </div>
        ...
        {isGenerating && (
          <div className="mt-2 md:mt-4 text-yellow-400/70 text-xs font-mono">
      ...
      {!selectedCategory && !currentTheory && (
        <div className={`
          text-center text-green-400/50 font-mono text-sm border border-green-400/30 
          ${isMobile ? 'p-4' : 'p-6'} rounded-none
        `}>
